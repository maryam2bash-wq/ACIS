    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          {/* ── Radar Chart — 15 Sovereign Layers ── */}
          <div className="rounded border border-orange-500/20 bg-card overflow-hidden">
            <div className="px-3 py-2 border-b border-orange-500/10 flex items-center justify-between">
              <span className="text-[10px] font-mono text-orange-400/60">radar · health</span>
              <span className="text-xs font-bold text-orange-400">خريطة صحة الطبقات</span>
            </div>
            <div style={{ height: 230 }}>
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart
                  data={SOVEREIGN_LAYERS.map((layer, i) => {
                    const ls = layerStatuses.find(l => l.id === layer.id);
                    return {
                      subject: `L${String(layer.id).padStart(2, "0")}`,
                      health: ls?.health ?? (90 + Math.round(Math.sin(i * 0.85) * 5)),
                      name: layer.name,
                    };
                  })}
                ...
                >
                  ...
                  <PolarAngleAxis
                    dataKey="subject"
                    tick={{ fill: "rgba(249,115,22,0.55)", fontSize: 9, fontFamily: "monospace" }}
                  />
                  <PolarRadiusAxis
                    angle={90} domain={[60, 100]} tick={false}
                    axisLine={false}
                  />
                  <Radar
                    name="الصحة"
                    ...
                    strokeWidth={1.5}
                    dot={{ fill: "rgba(249,115,22,0.7)", r: 2.5 }}
                  />
                  <RTooltip
                    contentStyle={{ background: "#111", border: "1px solid rgba(249,115,22,0.25)", borderRadius: "6px", fontSize: "11px" }}
                    formatter={(val: any, _: any, item: any) => [
                      `${val}%`,
                      SOVEREIGN_LAYERS.find(l => `L${String(l.id).padStart(2, "0")}` === item.payload?.subject)?.name ?? item.payload?.subject,
                    ]}
                  />
                </RadarChart>
              </ResponsiveContainer>
