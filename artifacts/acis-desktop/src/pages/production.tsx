...
import {
  ...
  Wand, FileJson, FileText as FileMd, Rocket, StopCircle,
} from "lucide-react";
...
export default function ProductionPage() {
  ...
  const [directorPhase, setDirectorPhase] = useState<string | null>(null);
  const [directorPhaseIdx, setDirectorPhaseIdx] = useState(0);
  ...
  // ── AI Director: تشغيل تلقائي عبر الـ backend ──────────────────────────
  const DIRECTOR_PHASE_LABELS: Record<string, string> = {
    script: "السيناريو", storyboard: "اللوحة المصورة", audio: "الصوت",
    images: "الصور", music: "الموسيقى", assembly: "التجميع النهائي",
  };
  const DIRECTOR_PHASES_ORDER = ["script", "storyboard", "audio", "images", "music", "assembly"];

  const handleAutoRun = useCallback(async () => {
    if (!selectedProject || directorRunning) return;
    const tid = toast.loading("🎬 AI Director يبدأ… جارٍ تشغيل جميع المراحل تلقائياً");
    setDirectorRunning(true);
    setDirectorPhaseIdx(0);
    setDirectorPhase(DIRECTOR_PHASES_ORDER[0]);

    try {
      const res = await fetch(`${API_BASE}/production/projects/${selectedProject}/auto-run`, { method: "POST" });
      if (!res.ok) throw new Error("فشل تشغيل AI Director");

      let phaseIdx = 0;
      let lastJobCount = projectJobs?.length || 0;

      while (phaseIdx < DIRECTOR_PHASES_ORDER.length) {
        setDirectorPhase(DIRECTOR_PHASES_ORDER[phaseIdx]);
        setDirectorPhaseIdx(phaseIdx);
        toast.loading(`🎬 AI Director — ${DIRECTOR_PHASE_LABELS[DIRECTOR_PHASES_ORDER[phaseIdx]]} (${phaseIdx + 1}/${DIRECTOR_PHASES_ORDER.length})`, { id: tid });
        ...
        while (waited < 300) {
          ...
          refetchJobs();
          refetchProjects();
          const jobsRes = await fetch(`${API_BASE}/production/projects/${selectedProject}/jobs`);
          const jobs = await jobsRes.json();
          const phaseJobs = jobs.filter((j: any) => j.phase === DIRECTOR_PHASES_ORDER[phaseIdx]);
          const latestJob = phaseJobs[0];
          if (latestJob?.status === "completed" || latestJob?.status === "failed") break;
          if (jobs.length > lastJobCount) lastJobCount = jobs.length;
        }
        phaseIdx++;
      }

      toast.success("🎬 AI Director اكتمل! جميع المراحل تمت بنجاح ✓", { id: tid });
      qc.invalidateQueries();
    } catch (err: any) {
      toast.error(`فشل AI Director: ${err?.message || "خطأ"}`, { id: tid });
    } finally {
      setDirectorRunning(false);
      setDirectorPhase(null);
      setDirectorPhaseIdx(0);
    }
  }, [selectedProject, directorRunning, projectJobs, refetchJobs, refetchProjects, qc]);

  // ── Export ──────────────────────────────────────────────────────────────
  const handleExport = useCallback(async (format: "markdown" | "json") => {
    if (!selectedProject) return;
    const url = `${API_BASE}/production/projects/${selectedProject}/export/${format}`;
    ...
    toast.success(`جارٍ تصدير المشروع كـ ${format === "markdown" ? "Markdown" : "JSON"} ↓`);
  }, [selectedProject]);

  const TABS = [
    { key: "projects", label: "المشاريع",      icon: Film },
    { key: "wizard",   label: "معالج ذكي ✨",  icon: Wand2 },
      ...
      {tab === "projects" && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {projects && projects.length > 0 && (
            <div className="lg:col-span-3 rounded border border-purple-500/20 bg-purple-500/5 overflow-hidden">
              <div className="flex items-center justify-between px-4 py-2.5 border-b border-purple-500/20 bg-purple-500/8">
                <span className="text-[10px] font-mono text-muted-foreground">7 مراحل · الأخضر = مكتمل · النابض = جارٍ · الرمادي = منتظر</span>
                <div className="flex items-center gap-2">
                  <ListVideo size={13} className="text-purple-400" />
                  <span className="text-sm font-semibold text-purple-400">الخط الزمني — Gantt</span>
                </div>
              </div>
              <div className="p-3 space-y-2" dir="rtl">
                {projects.slice(0, 5).map(p => (
                  <div key={p.id} className="flex items-center gap-3">
                    <div className="text-xs font-mono text-right truncate w-32 shrink-0 text-muted-foreground" title={p.title_ar || p.title}>
                      {(p.title_ar || p.title || "").substring(0, 12)}{(p.title_ar || p.title || "").length > 12 ? "…" : ""}
                    </div>
                    <div className="flex-1 flex items-center gap-0.5">
                      {PHASE_LABELS.map((label, i) => {
                        const done   = i < (p.phase ?? 0);
                        const active = i === (p.phase ?? 0) && p.status === "processing";
                        const failed = p.status === "failed" && i === (p.phase ?? 0);
                        const PIcon  = PHASE_ICONS[i];
                        return (
                          <div key={label}
                            title={label}
                            className={`flex-1 h-6 flex items-center justify-center rounded-sm text-[9px] font-mono transition-all ${
                              failed  ? "bg-red-500/30 text-red-400 border border-red-500/30" :
                              done    ? "bg-emerald-500/25 text-emerald-400 border border-emerald-500/20" :
                              active  ? "bg-purple-500/40 text-purple-300 border border-purple-400/40 animate-pulse" :
                    ...
                    <div className={`text-[9px] font-mono w-14 text-left shrink-0 ${
                      p.status === "completed" ? "text-emerald-400" :
                      p.status === "processing" ? "text-purple-400 animate-pulse" :
                      p.status === "failed" ? "text-red-400" : "text-muted-foreground/50"
                    }`}>
                      {p.status === "completed" ? "✓ مكتمل" : p.status === "processing" ? "● جارٍ" : p.status === "failed" ? "✗ فشل" : "○ معلق"}
                    </div>
                  </div>
                ))}
                {projects.length > 5 && (
                  <div className="text-[10px] font-mono text-muted-foreground/40 text-center">+ {projects.length - 5} مشاريع إضافية</div>
                )}
              </div>
            </div>
          )}
          ...
          <div className="space-y-4">
            {projectData ? (
              <>
                ...
                {/* ── AI Director Panel ── */}
                <div className="rounded-xl border-2 border-purple-500/40 bg-gradient-to-b from-purple-500/10 to-purple-900/5 overflow-hidden">
                  <div className="px-4 py-3 bg-purple-500/10 border-b border-purple-500/30 flex items-center justify-between">
                    <div className="flex gap-1">
                      {(["markdown", "json"] as const).map(fmt => (
                        <button key={fmt} onClick={() => handleExport(fmt)}
                          disabled={!projectJobs?.some((j: any) => j.status === "completed")}
                          title={`تصدير ${fmt}`}
                          className="flex items-center gap-1 px-2 py-1 rounded border border-purple-500/30 bg-purple-500/10 text-purple-300 text-[10px] font-mono hover:bg-purple-500/20 transition-colors disabled:opacity-30">
                          {fmt === "markdown" ? <FileMd size={9} /> : <FileJson size={9} />}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-bold text-purple-300">AI Director</span>
                    </div>
                  </div>
                  <div className="p-4 space-y-3">
                    <div className="font-bold text-right">{(projectData as any).title_ar || projectData.title}</div>
                    <p className="text-xs text-muted-foreground text-right leading-relaxed border-r-2 border-purple-500/30 pr-2">
                      {projectData.story_prompt?.substring(0, 120)}{(projectData.story_prompt?.length ?? 0) > 120 ? "…" : ""}
                    </p>
                    ...
                    {directorRunning && (
                      <div className="space-y-2">
                        <div className="flex items-center justify-between text-xs font-mono">
                          <span className="text-purple-400 animate-pulse">● يعمل</span>
                          <span className="text-muted-foreground">{directorPhaseIdx + 1} / 6</span>
                        </div>
                        <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
                          <div className="h-full bg-purple-500 rounded-full transition-all duration-1000"
                            style={{ width: `${((directorPhaseIdx) / 6) * 100}%` }} />
                        </div>
                        <div className="text-xs text-purple-300 font-mono text-right animate-pulse">
                          {directorPhase ? `🎬 ${DIRECTOR_PHASE_LABELS[directorPhase] || directorPhase}` : "جارٍ…"}
                        </div>
                        {/* Phase dots */}
                        <div className="flex gap-1 justify-end flex-row-reverse">
                          {DIRECTOR_PHASES_ORDER.map((ph, idx) => (
                            <div key={ph} title={(DIRECTOR_PHASE_LABELS as any)[ph]}
                              className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-mono border transition-all ${
                                idx < directorPhaseIdx ? "bg-emerald-500/20 border-emerald-500/50 text-emerald-400" :
                                idx === directorPhaseIdx ? "bg-purple-500/30 border-purple-400 text-purple-300 animate-pulse" :
                                "bg-secondary border-border/30 text-muted-foreground/30"
                              }`}>
                              {idx < directorPhaseIdx ? "✓" : idx + 1}
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <button
                      ...
                      disabled={directorRunning || generating || runningAll}
                      className="w-full flex items-center justify-center gap-2 py-3 rounded-lg bg-purple-600/20 border-2 border-purple-500/50 hover:bg-purple-600/30 hover:border-purple-400/70 text-purple-200 font-bold text-sm transition-all disabled:opacity-40 disabled:cursor-not-allowed">
                      {directorRunning
                        ? <><StopCircle size={15} className="text-purple-400" /> جارٍ… يمكنك مراقبة المراحل أدناه</>
                        : <><Rocket size={15} className="text-purple-400" /> تشغيل AI Director — جميع المراحل</>}
                    </button>
                    ...
                    <div className="space-y-2">
                      {ALL_PHASES.map(action => (
                        <Button key={action.type} onClick={() => handleGenerate(projectData.id, action.type)}
                          disabled={generating || runningAll || directorRunning}
