...
export default function NexusPage() {
  ...
  const TABS = [
    ...
    { key: "matrix",    label: "مصفوفة الأولويات", icon: Layers },
      ...
      {tab === "matrix" && (
        <div className="space-y-4" dir="rtl">
          <div className="flex items-center justify-between">
            <div className="text-xs font-mono text-muted-foreground">انقر على أي مهمة لعرض نتيجتها · التصنيف يعتمد على الأولوية والنوع</div>
            <div className="text-xs font-mono text-emerald-400/60">مصفوفة أيزنهاور</div>
          </div>
          {tLoad ? (
            <div className="grid grid-cols-2 gap-4">
              {Array(4).fill(0).map((_, i) => <Skeleton key={i} className="h-40 bg-card" />)}
            </div>
          ) : (
              ...
              {[
                {
                  ...
                  filter: (t: any) => (t.priority === "urgent") || (t.priority === "high" && t.status === "running"),
                },
                {
                  label: "جدوِل",
                  sub: "غير عاجل + مهم",
                  ...
                  filter: (t: any) => t.priority === "high" && t.status !== "running",
                },
                {
                  label: "فوِّض",
                  sub: "عاجل + غير مهم",
                ...
                {
                  label: "اقتل",
                  sub: "غير عاجل + غير مهم",
                  color: "border-border/40 bg-secondary/20",
                  badge: "bg-secondary text-muted-foreground border-border/40",
                  filter: (t: any) => t.priority === "low" || (!t.priority),
                },
              ].map(quadrant => {
                const qTasks = (tasks ?? []).filter(quadrant.filter).filter(t => t.status !== "completed");
                return (
                  <div key={quadrant.label} className={`rounded-xl border p-3 flex flex-col gap-2 ${quadrant.color}`}>
                    <div className="flex items-center justify-between mb-1">
                      <span className={`text-[9px] font-mono px-1.5 py-0.5 rounded border ${quadrant.badge}`}>{qTasks.length}</span>
                      <div className="text-right">
                        <div className="font-bold text-sm">{quadrant.label}</div>
                        <div className="text-[10px] text-muted-foreground font-mono">{quadrant.sub}</div>
                      </div>
                    </div>
                    {qTasks.length === 0 ? (
                      <div className="flex-1 flex items-center justify-center text-xs text-muted-foreground/40 font-mono">— فارغ —</div>
                    ) : (
                      <div className="space-y-1.5 overflow-y-auto max-h-44">
                        {qTasks.map(t => {
                          const Icon = TASK_TYPE_ICONS[t.type] ?? Brain;
                          return (
                            <button key={t.id}
                              onClick={() => setViewTaskId(t.id)}
                              className="w-full text-right p-2 rounded border border-border/30 bg-card/60 hover:border-emerald-400/30 hover:bg-emerald-400/5 transition-all text-xs">
                              <div className="flex items-center gap-1.5 justify-end mb-0.5">
                                <span className="font-semibold truncate">{t.title}</span>
                                <Icon size={10} className="text-muted-foreground/60 shrink-0" />
                              </div>
                              <div className={`text-[9px] font-mono ${STATUS_STYLES[t.status]?.split(" ")[0] ?? "text-muted-foreground"}`}>
          ...
          {!tLoad && tasks && tasks.filter(t => t.status === "completed").length > 0 && (
            <div className="rounded border border-emerald-400/20 bg-emerald-400/5 p-3 text-right">
              <div className="text-xs font-mono text-emerald-400 mb-1">{tasks.filter(t => t.status === "completed").length} مهمة مكتملة (مخفية من المصفوفة)</div>
              <button onClick={() => setTab("kanban")} className="text-[10px] font-mono text-muted-foreground hover:text-emerald-400 transition-colors">
                ← عرضها في الكانبان
              </button>
            </div>
          )}
