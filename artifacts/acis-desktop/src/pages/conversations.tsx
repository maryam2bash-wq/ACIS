...
import {
  ...
  User, Clock, Zap, X, Trash2, Search,
} from "lucide-react";
    ...
    <div className="flex h-[calc(100vh-8rem)] gap-4" dir="rtl">
      ...
      <div className="w-72 shrink-0 flex flex-col gap-3">
        ...
        <div className="relative">
          <Search size={11} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-muted-foreground/50" />
          <input
            value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)}
            placeholder="بحث في المحادثات…"
            className="w-full bg-secondary/50 border border-border/40 rounded-lg pr-7 pl-3 py-1.5 text-xs focus:outline-none focus:border-primary/40 transition-colors"
            dir="rtl"
          />
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-1">
          {cLoad ? (
            ...
            return (
              ...
              agent?.nameAr?.toLowerCase().includes(q) ||
