...
router.post("/projects/:projectId/auto-run", async (req, res) => {
  const { projectId } = req.params;
  const [project] = await db.select().from(projectsTable).where(eq(projectsTable.id, projectId));
  if (!project) return res.status(404).json({ error: "المشروع غير موجود" });

  const DIRECTOR_PHASES = ["script", "storyboard", "audio", "images", "music", "assembly"];

  res.json({ started: true, phases: DIRECTOR_PHASES, project_id: projectId });

  // تشغيل في الخلفية
  setImmediate(async () => {
    broadcast("director_started", { projectId, totalPhases: DIRECTOR_PHASES.length, projectName: project.titleAr || project.title });
    await db.insert(activityTable).values({
      id: randomUUID(), type: "agent_started",
      title: `AI Director بدأ: ${project.titleAr || project.title}`,
      description: `التشغيل التلقائي لـ ${DIRECTOR_PHASES.length} مراحل`,
    });

    for (let i = 0; i < DIRECTOR_PHASES.length; i++) {
      const phase = DIRECTOR_PHASES[i];
      broadcast("director_phase_start", { projectId, phase, phaseIndex: i, totalPhases: DIRECTOR_PHASES.length });
      ...
      while (attempts <= MAX_ATTEMPTS) {
        try {
          const jobIdLocal = randomUUID();
          jobId = jobIdLocal;

          await db.insert(generationJobsTable).values({
            id: jobIdLocal, project_id: projectId, phase,
            status: "running", model_used: PHASE_MODELS[phase] || "gemini-2.5-flash-lite",
            estimated_seconds: PHASE_TIMES[phase] || 60,
          });

          const phaseStatusMap: Record<string, string> = {
            script: "scripting", storyboard: "storyboard", audio: "generating",
            images: "generating", video: "generating", music: "generating", assembly: "post-production",
          };

          await db.update(projectsTable).set({
            status: phaseStatusMap[phase] || "generating",
            phase: i + 1,
            assets_generated: (project.assets_generated || 0) + i + 1,
            updated_at: new Date(),
          }).where(eq(projectsTable.id, projectId));

          // إعادة استخدام منطق generate الموجود عبر HTTP internal call
          const title = project.titleAr || project.title;
          const story = project.story_prompt || "";
          const lang  = project.language || "ar";
          const dur   = project.duration_seconds || 90;
          ...
          if (phase === "script") {
            const r = await callAIForTask("text_complex", AGENT_SYSTEM_PROMPTS["story-architect"],
              `اكتب سيناريو احترافي متكامل:\nعنوان: ${title}\nالقصة: ${story}\nاللغة: ${lang === "ar" ? "العربية" : lang}\nالنوع: ${projType}\nالمدة: ${dur}ث\n\n1. سينوبسيس\n2. الشخصيات\n3. السيناريو الكامل\n4. ملاحظات المخرج`);
            aiResult = r.text;
          } else if (phase === "storyboard") {
            const allJobs = await db.select().from(generationJobsTable).where(eq(generationJobsTable.project_id, projectId));
            const scriptJob = allJobs.find(j => j.phase === "script" && j.status === "completed");
            const [b, p] = await Promise.all([
              callAIForTask("text_complex", AGENT_SYSTEM_PROMPTS["visual-storyboard"],
                `صمم لوحة مصورة لـ 8-12 مشهداً:\nعنوان: ${title}\nالقصة: ${scriptJob?.result?.slice(0,800) || story}\nالمدة: ${dur}ث\nلكل مشهد: رقمه، الكاميرا، الإضاءة، الشخصيات، المزاج، المدة`),
              callAIForTask("text_complex", AGENT_SYSTEM_PROMPTS["ai-prompt-director"],
                `اكتب برومبت FLUX.1 Pro لـ 5 مشاهد رئيسية لـ "${title}".\nالقصة: ${story}\nلكل مشهد: برومبت إنجليزي مفصّل بالأسلوب البصري والإضاءة والكاميرا`),
            ]);
            aiResult = `═══ اللوحة المصورة ═══\n\n${b.text}\n\n═══ برومبت FLUX.1 ═══\n\n${p.text}`;
          } else if (phase === "audio") {
            const r = await callAIForTask("text_complex", AGENT_SYSTEM_PROMPTS["sound-music"],
              `صمم المشهد الصوتي لـ "${title}":\nالقصة: ${story}\nالمدة: ${dur}ث\n\n1. الثيم الصوتي\n2. المؤثرات الصوتية\n3. التوزيع الموسيقي\n\n[TTS_CHARACTERS_ONLY_START]\nحوار الشخصيات (3-4 جمل)\n[TTS_CHARACTERS_ONLY_END]`);
            aiResult = r.text;
            try {
              const ttsScript = (await import("../lib/media.js")).extractTtsScript(aiResult, 550);
              if (ttsScript?.length > 30) {
                const ttsResult = await callGeminiTTS(ttsScript, "Charon");
                if (ttsResult?.audioData) {
                  const fn = (await import("../lib/media.js")).saveTtsAudio(jobIdLocal, ttsResult.audioData, ttsResult.mimeType);
                  await db.update(generationJobsTable).set({ output_url: fn }).where(eq(generationJobsTable.id, jobIdLocal));
                }
              }
            } catch {}
          } else if (phase === "music") {
            const r = await callAIForTask("text_complex", AGENT_SYSTEM_PROMPTS["sound-music"],
              `أعدّ موسيقى تصويرية لـ "${title}":\nالقصة: ${story}\nالمدة: ${dur}ث\n\n1. الهوية الموسيقية\n2. توجيهات MusicGen\n3. جدول الموسيقى\n4. كلمات الشارة`);
            aiResult = r.text;
          } else if (phase === "assembly") {
            const allJobsForAssembly = await db.select().from(generationJobsTable).where(eq(generationJobsTable.project_id, projectId));
            const jobsByPhase: Record<string, any> = {};
            for (const j of allJobsForAssembly) {
              if (j.status === "completed" && j.phase && !jobsByPhase[j.phase]) jobsByPhase[j.phase] = j;
            }
            const ctx = ["script","storyboard","audio","music"].map(ph => jobsByPhase[ph]?.result ? `═══ ${ph} ═══\n${jobsByPhase[ph].result.slice(0,800)}` : "").filter(Boolean).join("\n\n");
            const r = await callAIForTask("text_complex", AGENT_SYSTEM_PROMPTS["timeline-assembly"],
              `أنت منسق التجميع النهائي لـ "${title}" (${dur}ث).\n\n${ctx}\n\n1. جدول المونتاج التفصيلي\n2. قائمة المشاهد\n3. توجيهات دمج الصوت\n4. انتقالات المونتاج\n5. ملاحظات ما بعد الإنتاج\n6. قائمة التسليم`);
            aiResult = r.text;
            try {
              const jobsForVideo = allJobsForAssembly;
              const byPh: Record<string, any> = {};
              for (const j of jobsForVideo) { if (j.status === "completed" && j.phase && !byPh[j.phase]) byPh[j.phase] = j; }
              const texts: Record<string, string> = {};
              if (byPh["storyboard"]?.result) texts.storyboard = byPh["storyboard"].result;
              if (byPh["script"]?.result) texts.script = byPh["script"].result;
              texts.assembly = aiResult;
              const scenes = (await import("../lib/video.js")).extractScenes(texts, dur);
              const vf = await (await import("../lib/video.js")).buildCompilationVideo({ jobId: jobIdLocal, projectTitle: title, totalDuration: dur, scenes, audioFilename: byPh["audio"]?.output_url });
              await db.update(generationJobsTable).set({ output_url: vf }).where(eq(generationJobsTable.id, jobIdLocal));
            } catch {}
          } else {
            const r = await callAIForTask("orchestration", AGENT_SYSTEM_PROMPTS["acis-master"],
              `أدر مرحلة "${phaseNames[phase] || phase}" للمشروع "${title}".\nالقصة: ${story}\nقدّم خطة تفصيلية وتقريراً.`);
            aiResult = r.text;
          }

          await db.update(generationJobsTable).set({ status: "completed", result: aiResult, completed_at: new Date() }).where(eq(generationJobsTable.id, jobIdLocal));

          broadcast("director_phase_done", {
            projectId, phase, phaseIndex: i, totalPhases: DIRECTOR_PHASES.length,
            projectName: title, resultLength: aiResult.length,
          });
          broadcast("project_updated", { projectId, phase, status: "completed" });
          break;
          ...
          if (attempts > MAX_ATTEMPTS) {
            if (jobId) {
              await db.update(generationJobsTable).set({
                status: "failed", result: `خطأ: ${err?.message || "فشل"}`, completed_at: new Date(),
              }).where(eq(generationJobsTable.id, jobId));
            }
            broadcast("director_phase_done", { projectId, phase, phaseIndex: i, failed: true });
          } else {
    ...
    // اكتمال المخرج
    await db.update(projectsTable).set({ status: "completed", phase: DIRECTOR_PHASES.length, updated_at: new Date() }).where(eq(projectsTable.id, projectId));
    await db.insert(activityTable).values({
      id: randomUUID(), type: "agent_completed",
      title: `AI Director اكتمل: ${project.titleAr || project.title}`,
      description: `اكتملت ${DIRECTOR_PHASES.length} مراحل تلقائياً`,
    });
    broadcast("director_complete", { projectId, projectName: project.titleAr || project.title, totalPhases: DIRECTOR_PHASES.length });
    broadcast("project_updated", { projectId });
    broadcast("activity_updated");
  });
});
...
router.get("/projects/:projectId/export/markdown", async (req, res) => {
  ...
  const phaseOrder = ["script","storyboard","audio","images","video","music","assembly"];
  const phaseAr: Record<string, string> = {
    script:"السيناريو", storyboard:"اللوحة المصورة", audio:"الصوت",
    images:"الصور", video:"الفيديو", music:"الموسيقى", assembly:"التجميع النهائي",
  };

  const jobsByPhase: Record<string, typeof jobs[0]> = {};
  for (const j of completedJobs) {
    if (j.phase && !jobsByPhase[j.phase]) jobsByPhase[j.phase] = j;
  }
  ...
  const lines: string[] = [
    `# ${title}`,
    ``,
    `> **المشروع:** ${project.title}${project.titleAr && project.titleAr !== project.title ? ` | ${project.titleAr}` : ""}`,
    `> **النوع:** ${project.type} | **المدة:** ${project.duration_seconds}ث | **اللغة:** ${project.language}`,
    `> **الحالة:** ${project.status} | **تاريخ الإنشاء:** ${project.created_at?.toISOString()?.split("T")[0] || "-"}`,
    ``,
    `---`,
    ``,
    `## فكرة المشروع`,
  ...
  for (const phase of phaseOrder) {
    const job = jobsByPhase[phase];
    if (!job?.result) continue;
    lines.push(``, `## ${phaseAr[phase] || phase}`, ``);
    lines.push(`> نُوِّلد بـ: **${job.model_used || "AI"}** | الحجم: ${(job.result.length / 1000).toFixed(1)}k حرف`);
    lines.push(``, job.result, ``);
    lines.push(`---`);
  }

  lines.push(``, `*تم التصدير من ACIS — نظام التكامل السينمائي بالذكاء الاصطناعي*`);

  const markdown = lines.join("\n");
  const filename = `${title.replace(/[^a-zA-Z0-9\u0600-\u06FF]/g, "_")}_ACIS.md`;
  res.setHeader("Content-Type", "text/markdown; charset=utf-8");
  res.setHeader("Content-Disposition", `attachment; filename="${encodeURIComponent(filename)}"`);
  res.send(markdown);
});

router.get("/projects/:projectId/export/json", async (req, res) => {
  const { projectId } = req.params;
  const [project] = await db.select().from(projectsTable).where(eq(projectsTable.id, projectId));
  if (!project) return res.status(404).json({ error: "المشروع غير موجود" });
  ...
  for (const j of completedJobs) {
    if (j.phase && !phases[j.phase]) {
      phases[j.phase] = {
        phase: j.phase, model_used: j.model_used, result: j.result,
  ...
  res.json({
    project: {
      id: project.id, title: project.title, titleAr: project.titleAr,
      type: project.type, status: project.status,
      story_prompt: project.story_prompt, language: project.language,
